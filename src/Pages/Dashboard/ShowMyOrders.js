    ...
    return (
        <tr>
            ...
            <td><div className='grid grid-cols-2 place-content-around'>
                <div>
                    {!order.paid && <label onClick={() => setDeleteOrder(order)} for="delete-modal" className="btn btn-circle   btn-primary">
                ...
                {(order.cost && !order?.paid) && <Link to={`/dashboard/payment/${order._id}`}><button className="btn mt-1 mr-4 btn-sm h-10 btn-outline btn-primary">Pay</button></Link>}
                {(order.price && order.paid) && <div>
                    <span className="btn mt-1 mx-2 btn-sm h-10  btn-neutral">Paid</span>
